...
public class CapeManager {
    ...
    public ICape parse(String name, Object object) {
        ...
        if(object instanceof String || object instanceof URL){
        	...
        	CapesLib.logger.error(String.format("Cape, %s, could not be parsed because it is not in an accepted format!", object));
        }
        return cape;
    }
    ...
    protected ICape parse(String name, String url) {
        ...
        try {
            ...
            CapesLib.logger.error(String.format("Are you crazy?? \"%s\" is not a valid URL!", url));
            e.printStackTrace();
        }
        return cape;
    }
