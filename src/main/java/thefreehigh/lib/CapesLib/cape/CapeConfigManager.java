...
protected static CapeConfigManager instance;
    ...
    static { availableIds.clear(availableIds.size()); }
    ...
    public CapeConfig parse(InputStream is) throws NullPointerException, JsonSyntaxException, IOException {
        if (is == null) {
            throw new NullPointerException("Can not parse a null input stream!");
        }
        return this.parse(IOUtils.toString(is, "UTF-8")); 
    }
    
    public CapeConfig parse(String jsonString) throws JsonSyntaxException {
